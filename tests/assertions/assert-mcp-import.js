...
if (!fs.existsSync(httpClientPath)) {
  console.error(
    `Not found: ${httpClientPath}. Did you run \"yarn generate:mcp\"?`,
  );
  process.exit(1);
}
...
const importRegex =
  /import\s*\{\s*mcpInstance\s*\}\s*from\s*['"](?:\.\.\/){2,4}mutators\/mcp-client(?:\.ts)?['"];?/;

if (!importRegex.test(content)) {
  console.error(
    'Assertion failed: mcpInstance import not found in http-client.ts',
  );
  process.exit(1);
}
